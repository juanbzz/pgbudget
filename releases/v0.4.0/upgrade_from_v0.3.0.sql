-- pgbudget v0.3.0 -> v0.4.0 Upgrade
-- Generated on Fri Sep 12 17:59:37 CDT 2025
-- This file upgrades an existing v0.3.0 installation

-- Version check
DO $$
BEGIN
    -- Check that we're running the expected version
    IF NOT EXISTS (
        SELECT 1 FROM utils.metadata
        WHERE key = 'version' AND value = 'v0.3.0'
    ) THEN
        RAISE EXCEPTION 'This upgrade requires pgbudget v0.3.0. Current version check failed.';
    END IF;
END $$;

-- Category Groups Feature Migrations
...
-- Upgrade complete
SELECT 'Upgrade to v0.4.0 completed successfully' AS result;
